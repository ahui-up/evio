...
import (
	...
	//"evio/internal"
	"github.com/jingchuang/evio/internal"
	//reuseport "evio/go_reuseport"
	reuseport "github.com/jingchuang/evio/go_reuseport"
	//"evio/config"
	"github.com/jingchuang/evio/config"
	//"github.com/jingchuang/evio/internal"
)
...
type conn struct {
	...
	connType ConnType //区分udp tcp 用于外部send时使用
}
...
func (c *conn) Wake() {
	...
	} else if c.connType == evUDP {
		//loopUDPRead(srv, c.loop, c.addrIndex, c.fd)
		if srv.events.Data != nil {
			out, _ := srv.events.Data(c, nil)
			if len(out) > 0 {
				// if s.events.PreWrite != nil {
				// 	s.events.PreWrite()
				// }
				//s.lns[c.addrIndex].pconn.WriteTo(out, c.remoteAddr)
				syscall.Sendto(c.fd, out[0:], 0, c.sa)
			}
			// case Shutdown:
			// 	return errClosing
		}
...
func (c *conn) Close() {
	if c.connType == evTCP {
		loopCloseConn(srv, c.loop, c, nil)
	}
}
func (c *conn) Send(data []byte) error {
	if c.connType == evTCP {
		...
		var err error
		err = syscall.Sendto(c.fd, data[0:], 0, c.sa)
		if err != nil {
			if err == syscall.EAGAIN {
				runtime.Gosched()
			} else {
...
func serve(events Events, listeners []*listener) error {
	...
	srv.balance = events.LoadBalance
	srv.tch = make(chan time.Duration)

	//println("-- server starting")
	if srv.events.Serving != nil {
		var svr Server
	...
	for i := 0; i < numLoops; i++ {
		...
		srv.loops = append(srv.loops, l)
	}
	// start loops in background
	srv.wg.Add(len(srv.loops))
	for _, l := range srv.loops {
		go loopRun(srv, l)
	}
	srvMtx.Unlock()
