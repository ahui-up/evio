...
func (c *stdudpconn) Send(data []byte) error {
	ts := len(data)
	t := 0
	n := 0
	var err error
	for t < ts {
		//n, err = syscall.Write(c.net.co, data[t:])
		n, err = s.lns[c.addrIndex].pconn.WriteTo(data, c.remoteAddr)
		if err != nil {
...
func (c *stdconn) Close() {
	stdloopClose(s, c.loop, c)
}

func (c *stdconn) Send(data []byte) error {
	ts := len(data)
	t := 0
	n := 0
	var err error
	for t < ts {
		//n, err = syscall.Write(c.conn, data[t:])
		n, err = c.conn.Write(data[t:])
		if err != nil {
			if err == syscall.EAGAIN {
				runtime.Gosched()
				continue
